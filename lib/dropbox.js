...
export async function dropboxJson(path, accessToken, body) {
  ...
  if (!response.ok) {
    const error = new Error(payload.error_summary || payload.error_description || JSON.stringify(payload) || "Dropbox API failed");
    error.statusCode = 500;
    error.dropboxStatus = response.status;
    error.dropboxPayload = payload;
    throw error;
  }
...
export async function uploadDropboxFile(accessToken, path, buffer, contentType) {
  ...
  if (!response.ok) {
    const error = new Error(payload.error_summary || payload.error_description || JSON.stringify(payload) || "Dropbox upload failed");
    error.statusCode = 500;
    error.dropboxStatus = response.status;
    error.dropboxPayload = payload;
    throw error;
