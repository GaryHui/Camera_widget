...
export async function getDropboxSharedUrl(accessToken, path, options = {}) {
  ...
  try {
    ...
    return formatDropboxUrl(created.url, raw);
  } catch (error) {
  ...
  return link ? formatDropboxUrl(link, raw) : null;
}

function formatDropboxUrl(url, raw) {
  if (!url) return null;
  if (!raw) return String(url).replace(/[?&]dl=1$/, "?dl=0");
